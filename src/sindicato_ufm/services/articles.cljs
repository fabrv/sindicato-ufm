...
(defn get-all [pgclient limit offset callback]
  (->
   (.query pgclient "select * from article order by created desc limit $1 offset $2" #js[limit offset])
...
(defn get-by-category [pgclient category limit offset callback]
  (->
   (.query pgclient "select * from article where category = $1 order by created desc limit $2 offset $3" #js[category limit offset])
   (.then (fn [a] (callback (js->clj (. a -rows)) nil)))
   (.catch (fn [e] (callback nil e)))))