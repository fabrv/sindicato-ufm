...
class App{
  ...
  mountRoutes(){
    ...
    router.get('json/categories/:category', (req: express.Request, res: express.Response) => {
    ...
    router.get('/json/articulo/:article', (req: express.Request, res: express.Response) => {
      pgClient.query(`SELECT * FROM public."ARTICLE" WHERE "headline" = '${req.params.article}'`, (error, result) => {
        if (error) {
          res.status(500).send(error)
        } else {
    ...
    router.get('/articulo/:article', (req: express.Request, res: express.Response) => {
      ...
      pgClient.query(`UPDATE public."ARTICLE" SET "views" = "views" + 1 WHERE "headline" = '${req.params.article}'; SELECT * FROM public."ARTICLE" WHERE "headline" = '${req.params.article}';`, (error, result: any) => {
        if (error) {
          res.status(500).send(error)
        } else {
          if (result[1].rowCount > 0) {
            console.log(`Articulo visitado: ${req.params.article}`)
...
function parseArticle(headline: string, subhead: string, body: string, date: string, author:string): string{
  const article = `
  <div class="content">
    <h1><a href="articulo/${encodeURIComponent(headline)}">${headline}</a></h1>
